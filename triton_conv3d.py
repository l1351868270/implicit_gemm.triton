...
def triton_conv3d(x: torch.Tensor, w: torch.Tensor, stride=(1, 1), padding=(0, 0), dilation=(1, 1)):
    N, D, H, W, C = x.shape
    K, T, R, S, C = w.shape
...
if __name__ == '__main__':
    ...
    conv3d = torch.nn.Conv3d(C, K, (T, R, S), stride=(stride_d, stride_h, stride_w), padding=(pad_d, pad_h, pad_w), dilation=(dila_d, dila_h, dila_w), bias=False).cuda().half()
    conv3d.weight.data = w
    y1 = conv3d(x)
    
    x_channel_last = x.permute(0, 2, 3, 4, 1).contiguous()
    w_channel_last = w.permute(0, 2, 3, 4, 1).contiguous()
    y2 = triton_conv3d(x_channel_last, w_channel_last, stride=(stride_d, stride_h, stride_w), padding=(pad_d, pad_h, pad_w), dilation=(dila_d, dila_h, dila_w))
    
    y1 = y1.permute(0, 2, 3, 4, 1).contiguous()
    # print(f'y1: shape:{y1.shape}, stride:{y1.stride()}')
    # print(f'y2: shape:{y2.shape}, stride:{y2.stride()}')
    if torch.allclose(y1, y2, atol=1e-2, rtol=1e-2):
        ...
        print(f'Triton: shape:{y2.shape}, stride:{y2.stride()}, {y2}')

    h_w = w_channel_last.cpu().numpy()
    h_x = x_channel_last.cpu().numpy()
    h_y1 = y1.detach().cpu().numpy()
    # print(f'h_w: shape:{h_w.shape}, stride:{h_w.strides}')
