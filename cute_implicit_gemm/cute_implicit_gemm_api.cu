...
void py_cute_implicit_gemm(torch::Tensor y, torch::Tensor x, torch::Tensor w, int pad_h, int pad_w, int U, int V, int dila_h, int dila_w) {
    ...
    bench::cute_implicit_gemm::cute_implicit_gemm<half>(y_hf, x_hf, w_hf, N, H, W, C, 
                       K, R, S, pad_h, pad_w, U, V, dila_h, dila_w);
    // bench::cute_implicit_gemm::cute_implicit_gemm<half>(y_hf, x_hf, w_hf, N, C, H, W, K, P, Q, R, S, U, V, pad_h, pad_w, dila_h, dila_w, "zeros");
}
